...
define([
	...
	function(declare, _WidgetBase, _TemplatedMixin, _WidgetsInTemplateMixin, Evented, template,  lang, query,
		...
		return declare([_WidgetBase, _TemplatedMixin, _WidgetsInTemplateMixin, Evented], {
			...
			_staticsData:function () {
                var featureSet = this._getFeatureSet();
                var CRMJ = 0, JZMJ = 0, GDZCTZ = 0;
                for (var i = 0; i < featureSet.features.length; i++){
                	var graphic = featureSet.features[i];
					CRMJ = CRMJ + graphic.attributes["CLMJ"];
                    JZMJ =  JZMJ + graphic.attributes["JZMJ"];
                    GDZCTZ =  GDZCTZ + graphic.attributes["GDZCTZ"];
				}
                ...
                select.onchange = function () {
                    if (select[select.selectedIndex].label == "出让面积"){
                        p.innerText  = "总和：  " + CRMJ.toFixed(2);
                    }
                    if (select[select.selectedIndex].label == "建筑面积"){
                        p.innerText = "总和：  " + JZMJ.toFixed(2);
                    }
                    if (select[select.selectedIndex].label == "固定资产投资"){
                        p.innerText = "总和：  " + GDZCTZ.toFixed(2);
                    }
                }
				//关闭
...
//			},
			...
			_onBtnMenuClicked: function(evt) {
				...
				this.featureActionManager.getSupportedActions(featureSet, layer).then(lang.hitch(this, function(actions) {
					...
					//统计
                    var staticsAction = new BaseFeatureAction({
                        name: "staticsDtat",
                        iconClass: 'icon-count',
                        label: '统计',
                        iconFormat: 'svg',
                        map: this.map,
                        onExecute: lang.hitch(this, this._staticsData)
                    });
                    staticsAction.name = "staticsDtat";
                    staticsAction.data = featureSet;
                    actions.push(staticsAction);
