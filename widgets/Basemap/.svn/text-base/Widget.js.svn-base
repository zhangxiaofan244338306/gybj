		...
		var clazz = declare([BaseWidget], {
			...
			startup: function() {
				...
				this.own(on(this.placehoder, 'click', lang.hitch(this, function() {
					if(document.getElementById("_44").children[0].className == "basemap_sl") {
						document.getElementById("_44").children[0].className = "basemap_yx";
						var layer0 = this.map.getLayer("basemap_yx");
						this.map.removeLayer(layer0);
						var baseLayer = new esri.layers.ArcGISTiledMapServiceLayer(this.appConfig.basemap_sl);
						baseLayer.id="basemap_sl";
						this.map.addLayer(baseLayer, 0);
					}else{
						document.getElementById("_44").children[0].className = "basemap_sl";
						var layer1 = this.map.getLayer("basemap_sl");
						this.map.removeLayer(layer1);
						var baseLayer = new esri.layers.ArcGISTiledMapServiceLayer(this.appConfig.basemap_yx);
						baseLayer.id="basemap_yx";
						this.map.addLayer(baseLayer, 0);
